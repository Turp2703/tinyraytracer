...
constexpr MMaterial      ivory = {1.0, {0.9,  0.5, 0.1, 0.0}, {0.4, 0.4, 0.3},   50.};
constexpr MMaterial      glass = {1.5, {0.0,  0.9, 0.1, 0.8}, {0.6, 0.7, 0.8},  125.};
constexpr MMaterial red_rubber = {1.0, {1.4,  0.3, 0.0, 0.0}, {0.3, 0.1, 0.1},   10.};
constexpr MMaterial     mirror = {1.0, {0.0, 16.0, 0.8, 0.0}, {1.0, 1.0, 1.0}, 1425.};
...
std::tuple<bool,vec3,vec3,MMaterial> scene_intersect(const vec3 &orig, const vec3 &dir) {
    vec3 pt, N;
    MMaterial material;
    ...
    while(!WindowShouldClose())
    {
        BeginDrawing();
            ...
            for (int i = 0; i < height*width; ++i){
                ...
                if (max>1) c = c*(1./max);
                DrawPixel(i % width, i / width, {(unsigned char)(255 * std::max(0.f, std::min(1.f, c[0]))), (unsigned char)(255 * std::max(0.f, std::min(1.f, c[1]))), (unsigned char)(255 * std::max(0.f, std::min(1.f, c[2]))), 255} );
            }
